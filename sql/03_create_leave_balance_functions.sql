-- Create missing leave balance functions
-- Run this script in Supabase SQL Editor to fix the missing function error

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS public.update_leave_balance_with_splitting(UUID, UUID, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS public.update_leave_balance(UUID, UUID, INTEGER, INTEGER);

-- Create the basic update_leave_balance function
...
-- Create the advanced function for smart splitting of leave balance
...
COMMENT ON FUNCTION public.update_leave_balance(UUID, UUID, INTEGER, INTEGER) IS 
...
COMMENT ON FUNCTION public.update_leave_balance_with_splitting(UUID, UUID, INTEGER, INTEGER) IS 
'Updates leave balance with smart splitting. Automatically uses deferred balance first, then current year balance if needed.';

-- Grant permissions
GRANT EXECUTE ON FUNCTION public.update_leave_balance(UUID, UUID, INTEGER, INTEGER) TO authenticated;
